...
4. **Log in.** A fresh install ships with a ready-made admin account:
   ...
   password in **Settings → Accounts** — you must, before hosting over the
   internet — or drop it to an ordinary player with
   `UPDATE login SET group_id = 0 WHERE userid = 'ragnarok';`.
